...
const Game = () => {
  const Player = (symbol) => {
    ...
    const addName = () => {
      name = window.prompt(`Enter Player ${playerNumber} name`);
      console.log(name);
    };
    const getName = () => {
      return name;
    };
    return { markLocation, addName, getName };
  };
  ...
  const checkWin = () => {
    ...
    if (
      ((board[1][1] === board[0][0] && board[2][2] === board[1][1]) ||
        (board[0][2] === board[0][0] && board[2][0] === board[1][1])) &&
      board[1][1] != 0
    ) {
      announceWin();
      return;
  ...
  const drawBoard = () => {
    if (winner) {
      document.getElementById(
        "turn"
      ).innerHTML = `${winner.getName()} has won! New game?`;
    } else {
  ...
  const checkPosition = (row, col) => {
    ...
    switch (value) {
      case 2:
        return '<img src="icons/p2.svg">';
      case 1:
        return '<img src="icons/p1.svg">';
      default:
        return `<button onClick='game.Click(${row},${col})' class='gameButton'>0</button>`;
    }
  };
